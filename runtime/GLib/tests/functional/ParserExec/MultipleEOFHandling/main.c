...
static void
test_functional_t_parser_exec_multiple_eofhandling(void) {
    ...
    TestContextProg *context = test_t_parser_parse_prog(parser, &parser_error);
    if (context==NULL) {
        if (parser_error) {
...
static void
test_functional_t_parser_exec_multiple_eofhandling_test (void)
{
  ...
  if (g_test_subprocess ())
    {
      test_functional_t_parser_exec_multiple_eofhandling();
      return;
    }
...
int main(int argc, char* argv[]) {
    ...
    g_test_add_func("/test", test_functional_t_parser_exec_multiple_eofhandling_test);
