...
{
    object->val = g_ptr_array_new();
    object->intToken = NULL;
    object->floatToken = NULL;
    object->_tset26 = NULL;
}
...
GList* test_context_b_token_get_float(TestContextB* self) {
    return antlr_parser_rule_context_get_tokens(ANTLR_PARSER_RULE_CONTEXT(self), TEST_T_PARSER_FLOAT);
}
AntlrTerminalNode *test_context_b_at_token_get_float(TestContextB* self, size_t i) {
    return antlr_parser_rule_context_get_token(ANTLR_PARSER_RULE_CONTEXT(self), TEST_T_PARSER_FLOAT, i);
}
...
TestContextB* test_t_parser_parse_b(TestTParser* self, GError **error) {
    ...
    || _la == TEST_T_PARSER_FLOAT) {
      antlr_recognizer_set_state(ANTLR_RECOGNIZER(self), 14);
      TEST_CONTEXT_B(local_context)->_tset26 = antlr_token_stream_LT(ANTLR_TOKEN_STREAM(ANTLR_PARSER(self)->input), 1);
      _la = antlr_int_stream_LA(ANTLR_INT_STREAM(ANTLR_PARSER(self)->input), 1);
      if (!(_la == TEST_T_PARSER_INT

      || _la == TEST_T_PARSER_FLOAT)) {
        TEST_CONTEXT_B(local_context)->_tset26 = antlr_error_strategy_recover_inline(ANTLR_PARSER(self)->err_handler, ANTLR_PARSER(self));
      } else {
        antlr_parser_consume(ANTLR_PARSER(self));
      }
      g_ptr_array_add(TEST_CONTEXT_B(local_context)->val, (gpointer)TEST_CONTEXT_B(local_context)->_tset26);
