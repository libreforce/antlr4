...
static void
test_functional_t_parser_exec_prediction_issue334(void) {
    ...
    TestContextFile *context = test_t_parser_parse_file_(parser, &parser_error);
    if (context==NULL) {
        if (parser_error) {
...
static void
test_functional_t_parser_exec_prediction_issue334_test (void)
{
  ...
  if (g_test_subprocess ())
    {
      test_functional_t_parser_exec_prediction_issue334();
      return;
    }
...
int main(int argc, char* argv[]) {
    ...
    g_test_add_func("/test", test_functional_t_parser_exec_prediction_issue334_test);
