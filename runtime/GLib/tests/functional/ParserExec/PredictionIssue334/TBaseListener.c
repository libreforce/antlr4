...
static void test_t_base_listener_interface_t_listener_enter_file_(TestTListener *self, TestContextFile *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("enter_file_ => %s\n", str_text);
    g_free(str_text);
#endif
}
static void test_t_base_listener_interface_t_listener_exit_file_(TestTListener *self, TestContextFile *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("exit_file_ => %s\n", str_text);
    g_free(str_text);
#endif
}

static void test_t_base_listener_interface_t_listener_enter_item(TestTListener *self, TestContextItem *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("enter_item => %s\n", str_text);
    g_free(str_text);
#endif
}
static void test_t_base_listener_interface_t_listener_exit_item(TestTListener *self, TestContextItem *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("exit_item => %s\n", str_text);
    g_free(str_text);
...
{
    iface->enter_file_ = test_t_base_listener_interface_t_listener_enter_file_;
    iface->exit_file_ = test_t_base_listener_interface_t_listener_exit_file_;

    iface->enter_item = test_t_base_listener_interface_t_listener_enter_item;
    iface->exit_item = test_t_base_listener_interface_t_listener_exit_item;
