...
static void
test_functional_t_parser_exec_label_aliasing_across_labeled_alternatives(void) {
    ...
    TestContextStart *context = test_t_parser_parse_start(parser, &parser_error);
    if (context==NULL) {
        if (parser_error) {
...
static void
test_functional_t_parser_exec_label_aliasing_across_labeled_alternatives_test (void)
{
  ...
  if (g_test_subprocess ())
    {
      test_functional_t_parser_exec_label_aliasing_across_labeled_alternatives();
      return;
    }
...
int main(int argc, char* argv[]) {
    ...
    g_test_add_func("/test", test_functional_t_parser_exec_label_aliasing_across_labeled_alternatives_test);
