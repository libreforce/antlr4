...
static void test_t_base_listener_interface_t_listener_enter_start(TestTListener *self, TestContextStart *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("enter_start => %s\n", str_text);
    g_free(str_text);
#endif
}
static void test_t_base_listener_interface_t_listener_exit_start(TestTListener *self, TestContextStart *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("exit_start => %s\n", str_text);
    g_free(str_text);
#endif
}

static void test_t_base_listener_interface_t_listener_enter_one(TestTListener *self, TestContextOne *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("enter_one => %s\n", str_text);
    g_free(str_text);
...
static void test_t_base_listener_interface_t_listener_exit_one(TestTListener *self, TestContextOne *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("exit_one => %s\n", str_text);
    g_free(str_text);
...
static void test_t_base_listener_interface_t_listener_enter_two(TestTListener *self, TestContextTwo *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("enter_two => %s\n", str_text);
    g_free(str_text);
...
static void test_t_base_listener_interface_t_listener_exit_two(TestTListener *self, TestContextTwo *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("exit_two => %s\n", str_text);
    g_free(str_text);
...
static void test_t_base_listener_interface_t_listener_enter_subrule(TestTListener *self, TestContextSubrule *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("enter_subrule => %s\n", str_text);
    g_free(str_text);
#endif
}
static void test_t_base_listener_interface_t_listener_exit_subrule(TestTListener *self, TestContextSubrule *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("exit_subrule => %s\n", str_text);
    g_free(str_text);
#endif
}
...
{
    iface->enter_start = test_t_base_listener_interface_t_listener_enter_start;
    iface->exit_start = test_t_base_listener_interface_t_listener_exit_start;

    iface->enter_one = test_t_base_listener_interface_t_listener_enter_one;
    iface->exit_one = test_t_base_listener_interface_t_listener_exit_one;

    iface->enter_two = test_t_base_listener_interface_t_listener_enter_two;
    iface->exit_two = test_t_base_listener_interface_t_listener_exit_two;

    iface->enter_subrule = test_t_base_listener_interface_t_listener_enter_subrule;
    iface->exit_subrule = test_t_base_listener_interface_t_listener_exit_subrule;
