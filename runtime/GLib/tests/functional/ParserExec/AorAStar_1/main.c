...
static void
test_functional_t_parser_exec_aor_astar_1(void) {

    const gchar *filename = g_strdup_printf("%s/%s", PACKAGE_SRC_DIR, "input_1.txt");
    GError *error = NULL;
...
static void
test_functional_t_parser_exec_aor_astar_1_test (void)
{
  ...
  if (g_test_subprocess ())
    {
      test_functional_t_parser_exec_aor_astar_1();
      return;
    }
...
int main(int argc, char* argv[]) {
    ...
    g_test_add_func("/test", test_functional_t_parser_exec_aor_astar_1_test);
