...
static void
test_functional_lexer_exec_position_adjusting_lexer(void) {
    ...
    TestPositionAdjustingLexer *lexer = test_position_adjusting_lexer_new_with_char_stream(ANTLR_CHAR_STREAM(file_stream));
...
static void
test_functional_lexer_exec_position_adjusting_lexer_test (void)
{
  ...
  if (g_test_subprocess ())
    {
      test_functional_lexer_exec_position_adjusting_lexer();
      return;
    }
...
int main(int argc, char* argv[]) {
    ...
    g_test_add_func("/test", test_functional_lexer_exec_position_adjusting_lexer_test);
