...
static void test_t_base_listener_interface_t_listener_enter_expression_list(TestTListener *self, TestContextExpressionList *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("enter_expression_list => %s\n", str_text);
    g_free(str_text);
#endif
}
static void test_t_base_listener_interface_t_listener_exit_expression_list(TestTListener *self, TestContextExpressionList *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("exit_expression_list => %s\n", str_text);
    g_free(str_text);
#endif
}

static void test_t_base_listener_interface_t_listener_enter_e(TestTListener *self, TestContextE *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("enter_e => %s\n", str_text);
    g_free(str_text);
...
static void test_t_base_listener_interface_t_listener_exit_e(TestTListener *self, TestContextE *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("exit_e => %s\n", str_text);
    g_free(str_text);
...
static void test_t_base_listener_interface_t_listener_enter_typespec(TestTListener *self, TestContextTypespec *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("enter_typespec => %s\n", str_text);
    g_free(str_text);
#endif
}
static void test_t_base_listener_interface_t_listener_exit_typespec(TestTListener *self, TestContextTypespec *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("exit_typespec => %s\n", str_text);
    g_free(str_text);
#endif
}
...
static void test_t_base_listener_interface_t_listener_init(TestTListenerInterface *iface)
{
    ...
    iface->enter_expression_list = test_t_base_listener_interface_t_listener_enter_expression_list;
    iface->exit_expression_list = test_t_base_listener_interface_t_listener_exit_expression_list;

    iface->enter_e = test_t_base_listener_interface_t_listener_enter_e;
    iface->exit_e = test_t_base_listener_interface_t_listener_exit_e;

    iface->enter_typespec = test_t_base_listener_interface_t_listener_enter_typespec;
    iface->exit_typespec = test_t_base_listener_interface_t_listener_exit_typespec;
