...
static void test_t_base_listener_interface_t_listener_enter_declarator(TestTListener *self, TestContextDeclarator *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("enter_declarator => %s\n", str_text);
    g_free(str_text);
#endif
}
static void test_t_base_listener_interface_t_listener_exit_declarator(TestTListener *self, TestContextDeclarator *ctx) {
#if ENABLE_TRACE
    gchar *str_text = antlr_parse_tree_get_text(ANTLR_PARSE_TREE(ctx));
    g_print("exit_declarator => %s\n", str_text);
    g_free(str_text);
#endif
}
...
static void test_t_base_listener_interface_t_listener_init(TestTListenerInterface *iface)
{
    ...
    iface->enter_declarator = test_t_base_listener_interface_t_listener_enter_declarator;
    iface->exit_declarator = test_t_base_listener_interface_t_listener_exit_declarator;
