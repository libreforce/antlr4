...
static guint test_t_parser_serialized_atn[] = {
  0x03, 0x0430, 0xD6D1, 0x8206, 0xAD2D, 0x4417, 0xAEF1, 0x8D80, 0xAADD, 0x03, 0x05, 0x0C, 0x04, 0x02, 0x09, 0x02, 0x04, 0x03, 0x09, 0x03, 0x03, 0x02, 0x03, 0x02, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x02, 0x02, 0x04, 0x02, 0x04, 0x02, 0x02, 0x09, 0x02, 0x06, 0x03, 0x02, 0x02, 0x02, 0x04, 0x08, 0x03, 0x02, 0x02, 0x02, 0x06, 0x07, 0x05, 0x04, 0x03, 0x02, 0x07, 0x03, 0x03, 0x02, 0x02, 0x02, 0x08, 0x09, 0x07, 0x03, 0x02, 0x02, 0x09, 0x0A, 0x07, 0x04, 0x02, 0x02, 0x0A, 0x05, 0x03, 0x02, 0x02, 0x02, 0x02
};
...
void test_context_s_class_parser_rule_context_enter_rule(AntlrParserRuleContext *self, AntlrParseTreeListener *listener) {
    test_t_listener_enter_s(TEST_T_LISTENER(listener), TEST_CONTEXT_S(self));
}

void test_context_s_class_parser_rule_context_exit_rule(AntlrParserRuleContext *self, AntlrParseTreeListener *listener) {
...
TestContextS* test_t_parser_parse_s(TestTParser* self, GError **error) {
    ...
    TEST_CONTEXT_S(local_context)->r =  (TestContextA*)ret;
    if (ret==NULL || (error && *error)) {
        return NULL;
    }
    ANTLR_PARSER(self)->ctx->stop = antlr_token_stream_LT(ANTLR_TOKEN_STREAM(ANTLR_PARSER(self)->input), -1);

    //<ToStringTree("TEST_CONTEXT_S(local_context)->r"):writeln()>
    gchar *str = antlr_tree_to_string_tree(ANTLR_TREE(local_context->r), antlr_recognizer_get_rule_names(ANTLR_RECOGNIZER(self)));
    g_print("%s\n", str);
    g_free(str);
